...
class PGExplainer(ExplainerAlgorithm):
    ...
    # def get_embeddings(x, edge_index, **kwargs):
    #     """Overridden version."""
    ...
    def train(
        ...
        **kwargs,
        ...
        values = torch.arange(x[0].shape[2])
        edge_index = torch.cartesian_prod(values, values)
        ...
        if self.model_config.task_level == ModelTaskLevel.node:
            hard_node_mask, hard_edge_mask = self._get_hard_masks(
                ...
                index,
                edge_index,
                num_nodes=x[0].shape[2],  # MIO: era x.size(0)
            )

            edge_mask = edge_mask[hard_edge_mask]

        y_hat, y = model(x, edge_index, **kwargs), target[..., 0]
        # y_hat = y_hat[0]  # MIO: non c'era
        ...
        # clear_masks(model) # MIO: non era commentato
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.model_config.task_level == ModelTaskLevel.node:
            ...
            _, _, N, _ = x.shape
            values = torch.arange(N)  # [N]
            edge_index = torch.cartesian_prod(values, values)  # [N*N, 2]
            edge_index = edge_index.transpose(0, 1)  # [2, N*N]
            ...
            hard_node_mask, hard_edge_mask = self._get_hard_masks(
                model, index, edge_index, num_nodes=N
            )
    ...
    def _get_inputs(
        ...
        if (
            self.model_config.task_level == ModelTaskLevel.node and False
        ):  # MIO: and False
            assert index is not None
            # embedding = torch.tensor(embedding)
            # a = embedding[index].view(1, -1)
            # print(a)
            # print(zs)
            # zs.append(a.repeat(zs[0].size(0), 1))
            zs.append(embedding[index].view(1, -1).repeat(zs[0].size(0), 1))
            # print(zs)
            # print(zs.shape)
        return torch.cat((zs[0], zs[1]), dim=-1)
