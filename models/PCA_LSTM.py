...
Original file is located at
    https://colab.research.google.com/drive/1w5gl-6-Z9lldXy_NNCIEdOhXu5YESR5e
"""
...
class PCA_LSTM(tf.keras.Model):
  ...
  def call(self, inputs):
    ...
    self.logger.critical('reduced_inputs: ' + str(reduced_inputs.shape))
    ...
    for h in range(H):
      x = inputs[:,h]
      x = tf.stack(self.pca.transform(tf.reshape(x, (B, N*F)))) # PCA converte [B,NF] in [B,N]
      self.logger.critical('x: ' + str(x.shape))
      memory, carry = self.cell(x, carry) # Memory: [B, N], Carry: [2, [B, N]]; memory e carry[0] sono identici
